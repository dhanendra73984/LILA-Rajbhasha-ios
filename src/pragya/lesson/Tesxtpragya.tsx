...
const Tesxtpragya = (props: any) => {
  ...
  const getInitialTimerValue = () => {
    if (modifiedArray.length > 0) {
      const firstModifiedArrayData = modifiedArray[0];
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        } else if (modifiedArray[0] === 'TestChoiceClue') {
...
// for fill  in the blanks 
  const handleNext = () => {
    ...
    if (currentIndex === testChoiceData.length - 1) {
      ...
      props.navigation.navigate('Testmcqinstructionpragya', {
        trueFalseCounter1: trueFalseCounter,
        fillInBlanksCounter1:fillInBlanksCounter,
        testChoiceCounter1:testChoiceCounter,
        ApiResponse,
        Package,
        Medium,
        selectedLessonIndex,
