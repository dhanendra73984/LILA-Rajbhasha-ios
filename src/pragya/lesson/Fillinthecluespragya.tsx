...
const Fillinthecluespragya = (props: any) => {
  const {
    exerciseName,
    exerciseContent,
    ApiResponse,
    ...
    const fetchData = async () => {
      try {
        const response = await axios.post(
          'https://lilaonmobile.rb-aai.in/LILAWebAPI/api/Home/getFillClues/',
          {
            LangSelected: Medium,
  ...
  const handleInputChange = (text: string) => {
    ...
    // Check if the input is non-empty and does not contain Hindi characters
  ...
  return (
    ...
    <View style={styles.container}>
      ...
      {fillCluesData.length > 0 && (
        <View  style={{height:200}}>
          <Text style={styles.sentence}>{currentIndex + 1}.{fillCluesData[currentIndex].HPreSentence}</Text>
          ...
          <TextInput
            ...
            <Text style={styles.sentence}>{fillCluesData[currentIndex].HPostSentence}</Text>
            <Text style={{color:'black',marginTop:2}}>{fillCluesData[currentIndex].LangClues}</Text>
          </View>
    ...
    <View style={styles.rowContainer}>
        ...
        <TouchableOpacity onPress={handleShowCorrectAnswer}>
              <Text style={styles.navigationButton} >{langWiseWords[0]}</Text>
        </TouchableOpacity>
        <TouchableOpacity onPress={handleClear}>
          <Text style={styles.navigationButton} >{langWiseWords[3]}</Text>
        </TouchableOpacity>
        <TouchableOpacity onPress={checkAnswer}>
          <Text style={styles.navigationButton}>{langWiseWords[2]}</Text>
      </TouchableOpacity>
...
export default Fillinthecluespragya
