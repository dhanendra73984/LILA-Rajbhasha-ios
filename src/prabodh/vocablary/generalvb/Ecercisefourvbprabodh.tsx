...
const Ecercisefourvbprabodh = (props:any) => {
...
case 'unchalis.jpg':
  ...
  const [wordData, setWordData] = useState<ServerData[]>([]);//containg all the data 
...
// Function to get image source based on WordPic
  ...
  //for genrating random images and ranodm text
    ...
    const fetchData = async () => {
      try {
        if (indexofgencategory >= 0 && indexofgencategory < data.length) {
          ...
          // Initialize random items and selected random item when data is fetched
          // const newRandomItems = (response.data || []).sort(() => 0.5 - Math.random()).slice(0, 4);
                const newRandomItems = (response.data || [])
                    .filter((item:any) => item.HWord !== null && item.WordPic !== null)
                    .sort(() => 0.5 - Math.random())
                    .slice(0, 4);
          const newRandomIndex = Math.floor(Math.random() * newRandomItems.length);
          const newSelectedRandomItem = newRandomItems[newRandomIndex];
  ...
  //handling the logic for rerenderig after pressing on correct image 
      const handleImagePress = (item: ServerData) => {
        if (item.HWord === selectedRandomItem?.HWord) {
          const newRandomItems = (wordData || [])
            .filter((item) => item.HWord !== null && item.WordPic !== null)
            .sort(() => 0.5 - Math.random())
            .slice(0, 4);
        ...
        console.log("Image pressed:", item.HWord);
        // You can add additional logic or actions based on the pressed item
      };
    ...
    <View style={{flex:1}}>
      <SafeAreaView style={{ backgroundColor: '#0D6EFD' }}>
                  <View style={styles.header}>
                        ...
                        <Text style={styles.headerTitle}>{ApiResponse[224]?.LangWiseWords}</Text>
                   ...
                   <ImageBackground
                      ...
                      <View style={styles.separator} />
                            ...
                            {/* Container for images, with each row holding two images */}
                             {/* Container for images, with each row holding two images */}
                            <View>
                            {/* Container for images, with each row holding two images */}
                              <View style={styles.imageContainer}>
                                {randomItems.map((item, index) => (
                                  item.HWord !== null && item.WordPic !== null && (
                                    <TouchableOpacity key={index} onPress={() => handleImagePress(item)}>
                                      <View style={styles.imageStyle}>
                                        <Image
                                          style={styles.imageStyle}
                                          source={item.WordPic ? getImageByWordPic(item.WordPic) : require('../../../../assets/img/baagh.jpg')}
                                        />
                                      </View>
                                    </TouchableOpacity>
                                  )
                            ...
                            {/* Display the sound in palysound button and then match that  */}
                            {/* <Text style={styles.forrandomtext}>{selectedRandomItem?.HWord}</Text> */}
...
export default Ecercisefourvbprabodh
