...
const Wordmeaningprabodh = (props:any) => {
        ...
        const selectedItemText= props.route.params.selectedItemText;
        
        const  SectionName= 'Words Meaning'
        const [wordData, setWordData] = useState<ServerData[]>([]);

        //this is for GenCategory object 
        const indexofgencategory = props.route.params.selectedItemIndex;
         ...
         if (indexofgencategory >= 0 && indexofgencategory < data.length) {
          ...
          console.log(selectedData); // This will print the corresponding data value
        } else {
        ...
        //   fetchData();
        // }, [Package, props.route.params.selectedItemText]);
      ...
      //rendering flatlist items here........
     

      //handling pop up modle
      const [selectedItem, setSelectedItem] = useState<ServerData | null>(null);
      const [isModalVisible, setModalVisible] = useState(false);
    
      const handleItemPress = (item: ServerData) => {
        setSelectedItem(item);
        setModalVisible(true);
      };
    //after onpress of listiem their should be sound button for that this is 
          const handleplaysound = async () => {
            // Assuming selectedItem contains your selected data
            const selectedWordSound = selectedItem?.WordSound;
          
            if (selectedWordSound) {
              // Replace ".wav" with ".mp3"
              const updatedWordSound = selectedWordSound.replace('.wav', '.mp3');
          
              // Log the updated URL
              console.log('Updated WordSound URL:', updatedWordSound);
          
              try {
                // Play the sound using react-native-sound-player
                await SoundPlayer.playUrl(`https://lilaonmobile.rb-aai.in/LILAMobileData/VocabSoundFiles/${updatedWordSound}`);
                console.log('Sound file played successfully');
              } catch (error) {
                console.error('Error playing sound file:', error);
              }
            } else {
              console.error('No WordSound available for the selected item');
            }
          };
      ...
      const renderItem = ({ item, index }: { item: ServerData; index: number }) => (
        <TouchableOpacity onPress={() => handleItemPress(item)}>
          <View>
            <Text style={styles.FlatListvalues}>{item.HWord}</Text>
          </View>
          {index === wordData.length - 1 && (
            <TouchableOpacity onPress={handleClosePressofflatlis}>
              <View style={styles.closeButtonforflatlist}>
                <Text style={styles.btntext} >{ApiResponse[45]?.LangWiseWords}</Text>
              </View>
            </TouchableOpacity>
          )}
        </TouchableOpacity>
      ...
      const  handleClosePressofflatlis=()=>{
        navigation.goBack()
      }
    ...
    <View style={{flex:1}}>
      <SafeAreaView style={{ backgroundColor: '#0D6EFD' }}>   
            <View style={styles.header}>
              ...
              <Text style={styles.headerTitle}>{ApiResponse[12]?.LangWiseWords}</Text>
            ...
            style={styles.backgroundImage}>
                        ...
                        {/* <FlatList
                          data={wordData}
                          renderItem={({ item }) => <Text>{item.HWord}</Text>}
                          keyExtractor={(item, index) => index.toString()}
                        /> */}

            <Text style={styles.titleofcategory}>{selectedItemText}</Text>
            ...
            <Modal isVisible={isModalVisible}>
                <View style={styles.modalContent}>
                  <Text style={styles.titleofpopup}>{selectedItem ? selectedItem.HWord : ''}:-{selectedItem ? selectedItem.LangWiseWords : ''}</Text>
                 {/* Add Image component with the source of your image */}
              {/* Construct the image source dynamically */}
              {/* <Image style={styles.imageStyle} source={{ uri: `../../../../assets/img/${selectedItem?.WordPic}` }} /> */}
              {/* <Image style={styles.imageStyle} source={require(`../../../../assets/img/${selectedItem?.WordPic}`)} /> */}
              <Image style={styles.imageStyle} source={getImageByWordPic(selectedItem?.WordPic)} />
                  ...
                  <View style={styles.buttonRow}>
                    <TouchableOpacity onPress={handleplaysound}>
                      <View style={styles.okButton}>
                        <Text style={styles.btntext}>{ApiResponse[15]?.LangWiseWords}</Text>
                      </View>
                    </TouchableOpacity>
                ...
                {/* <View>
                  ...
                  <Text>Wordmeaningprabodh handle here with pop upimage and sample</Text>
                </View> */}
            </ImageBackground>
...
export default Wordmeaningprabodh
