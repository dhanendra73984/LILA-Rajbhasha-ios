import { Button, FlatList, Image, ImageBackground, LogBox, SafeAreaView, ScrollView, StyleSheet, Text, TouchableOpacity, View } from 'react-native'
...
const Ecercisethreevbprabodh = (props:any) => {
...
case 'unchalis.jpg':
  ...
  const [wordData, setWordData] = useState<ServerData[]>([]);
  const [randomItems, setRandomItems] = useState<ServerData[]>([]);
  const [selectedRandomItem, setSelectedRandomItem] = useState<ServerData | null>(null);
 ...
 //for genrating random texts 
  ...
  //for handling sound presss event selectedRandomItem?.WordSound
  const handleSoundPress = async () => {
    try {
      // Replace ".wav" with ".mp3" in the selected word
      const soundNameWithExtension = selectedRandomItem?.WordSound.replace('.wav', '.mp3');
  ...
  const  handleCloseButtonPress=()=> {
      ...
      ? langWiseInstruction.replace(/<br>/gi, '\n')
      : '';
...
<SafeAreaView style={{ backgroundColor: '#0D6EFD' }}>   
                 <View style={styles.header}>
                        ...
                        <Text style={styles.headerTitle}>{ApiResponse[223]?.LangWiseWords}</Text>
                    ...
                    <ImageBackground
                      ...
                      <View style={styles.container}>
                          ...
                          <FlatList
                              data={randomItems.filter(item => item.HWord !== null && item.HWord.trim() !== '')}
                              numColumns={2}
                              keyExtractor={(item, index) => index.toString()}
                              renderItem={({ item }) => (
                                <TouchableOpacity onPress={() => handleTextPress(item.HWord)}>
                                  <View style={styles.textItem}>
                                    <Text style={styles.randomtext}>{item.HWord}</Text>
                                  </View>
                                </TouchableOpacity>
                              )}
                            />
                          ...
                          {/* Display the WordSound corresponding to the selected random item */}
                          <TouchableOpacity onPress={handleSoundPress}>
                            <View style={styles.closeButtonConatainer}>
                              <Text style={styles.closeButton}>{ApiResponse[15]?.LangWiseWords}</Text>
                            </View>
                          </TouchableOpacity>
...
export default Ecercisethreevbprabodh
      ...
      textItem: {            
       ...
       marginHorizontal: 50,   
         ...
         randomtext: {
          ...
          color: 'black',
          fontWeight: 'bold',
          marginTop: 5, // Adjust the top margin for space between texts
