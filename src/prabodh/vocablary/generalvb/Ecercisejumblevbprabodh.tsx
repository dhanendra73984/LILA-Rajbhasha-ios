import { Alert, Button, FlatList, Image, ImageBackground, LogBox, SafeAreaView, ScrollView, StyleSheet, Text, TouchableOpacity, View } from 'react-native'
...
const Ecercisejumblevbprabodh = (props:any) => {

  const getImageByWordPic = (wordPic:string) => {
    switch (wordPic) {
      case 'baagh.jpg':
        return baaghImage;
      case 'sher.jpg':
...
case 'unchalis.jpg':
  ...
  const [wordData, setWordData] = useState<ServerData[]>([]);//conating all the data 
  const [randomItems, setRandomItems] = useState<ServerData[]>([]);
  const [selectedRandomItem, setSelectedRandomItem] = useState<ServerData | null>(null);//for random genration handling
    ...
    const fetchData = async () => {
      try {
        if (indexofgencategory >= 0 && indexofgencategory < data.length) {
          ...
          updateRandomItems(response.data);
        } else {
 ...
 //for genrating random texts 
  const updateRandomItems = (data: ServerData[]) => {
    // Filter out items with null or empty HWord
    const filteredData = data.filter(item => item.HWord && item.HWord.trim() !== '');
  
    // If there are at least 4 valid items, proceed with updating
    if (filteredData.length >= 4) {
      const newRandomItems = [...filteredData].sort(() => 0.5 - Math.random()).slice(0, 4);
      const newSelectedRandomItem = newRandomItems[Math.floor(Math.random() * newRandomItems.length)];
      setRandomItems(newRandomItems);
      setSelectedRandomItem(newSelectedRandomItem);
    } else {
      // Handle the case where there are not enough valid items
      console.warn('Not enough valid items to update random items.');
    }
  };
  // const handleTextPress = (text: string) => {
  //   if (text === selectedRandomItem?.HWord) {
  //     updateRandomItems(wordData);
  //   }

  //   console.log("Text pressed:", text);
  //   // You can add additional logic or actions based on the pressed text
  // };
  // const handleTextPress = (word: string) => {
  //   console.log("Word pressed:", word);
  
  //   // Add the pressed word to the pressedWords array
  //   setPressedWords(prevWords => [...prevWords, word]);
  
  //   // Check if the pressedWords array matches the HWord of the selectedRandomItem
  //   const isMatch = pressedWords.join('') === selectedRandomItem?.HWord;
  
  //   if (isMatch) {
  //     // Update the random items
  //     const newRandomItems = wordData
  //       .filter(item => item.HWord && item.HWord.trim() !== '')
  ...
  useEffect(() => {
    ...
    // Always set the modal to visible
    if (isMatch) {
      setIsModalVisible(true);
      setFlag(1); // Set flag to 1 when a match is found
    }
  }, [pressedWords, selectedRandomItem]);
  ...
  const handleTextPress = (word: string) => {
    ...
    // Check if a match has already been found
    if (flag === 1) {
      // Show popup message in the modal
      setIsModalVisible(true);

      // Reset the flag after showing the modal
      setFlag(0);

      // Do not add the pressed word to the array
      return;
    }
  ...
  // Your function to close the modal
 ...
 //logic for next button..............
  const handleNextPress = () => {
    // Logic to get the next random item
    const newRandomItems = wordData
        .filter(item => item.HWord && item.HWord.trim() !== '')
        .sort(() => 0.5 - Math.random())
        .slice(0, 4);
  ...
  //fetching instuction data 
  const langWiseInstruction = wordData[0]?.LangWiseInstruction;
  const langWiseInstructionWithBlueSquares = langWiseInstruction
  ? langWiseInstruction.replace(/<br>/g, '\n'): '';
...
<SafeAreaView style={{ backgroundColor: '#0D6EFD' }}>
                 <View style={styles.header}>
                        ...
                        <Text style={styles.headerTitle}>{ApiResponse[226]?.LangWiseWords}</Text>
                    ...
                    <ImageBackground
                        ...
                        // source={require('../../../../assets/img/baagh.jpg')}
                        style={styles.backgroundImage}>

                       {/* instruction section here.......... */}
                       <Text style={styles.instruction}>
                          {langWiseInstructionWithBlueSquares.split('\n').map((line, index) => (
                            <Text key={index}>
                              {index > 0 && '\n'}{/* Add newline only if it's not the first line */}
                              <Text style={{ color: '#0D6EFD', fontSize: 10 }}>■ </Text>
                              {line}
                            </Text>
                          ))}
                        </Text>
                      ...
                      <View style={styles.container}>
                          {/* Display four random texts in rows with two texts per row */}
                                        {/* Display four random texts in rows with two texts per row */}
                           {/* Display words from HQuestion in rows */}
                            {selectedRandomItem && selectedRandomItem.HQuestion !== null && (
                              <View style={styles.textsContainer}>
                                {selectedRandomItem.HQuestion.split('/').map((word, index) => (
                                  <TouchableOpacity key={index} onPress={() => handleTextPress(word)}>
                                    <View style={styles.textItem}>
                                      <Text style={styles.randomtext}>{word}</Text>
                                    </View>
                                  </TouchableOpacity>
                                ))}
                              </View>
                         ...
                         {/* showing pressed item here */}
                         <View style={styles.pressedWordsContainer}>
                          <Text style={styles.pressedWordsText}>{pressedWords.join(' ')}</Text>
                        </View>

                          {/* Display the image corresponding to the selected random item */}
                          {selectedRandomItem?.WordPic && (
                            <Image
                              style={styles.imageStyle}
                              source={getImageByWordPic(selectedRandomItem.WordPic)}
                            />
                          )}
                          ...
                          <TouchableOpacity onPress={handleCloseButtonPress}>
                            <View style={styles.closeButtonConatainer}>                           
                              <Text  style={styles.closeButton} >{ApiResponse[45]?.LangWiseWords}</Text>                               
                            </View>
                            </TouchableOpacity>
                            ...
                            <View style={styles.closeButtonConatainer}>
                              <Text style={styles.closeButton}>{ApiResponse[3]?.LangWiseWords}</Text>
                            </View>
                          </TouchableOpacity>
                          ...
                          <TouchableOpacity onPress={handleNextPress}>
                            <View style={styles.closeButtonConatainer}>
                              <Text style={styles.closeButton}>{ApiResponse[19]?.LangWiseWords}</Text>
                            </View>
                          </TouchableOpacity>
                          </View>
                        ...
                        <Modal
                          ...
                          >
                            <View style={styles.modalContainer}>
                              ...
                              <Text style={styles.modalText}>{ApiResponse[0]?.LangWiseWords}</Text>
                              <TouchableOpacity onPress={closeModal}>
                                <Text style={styles.modalCloseButton}>{ApiResponse[45]?.LangWiseWords}</Text>
                              </TouchableOpacity>
                            </View>
                          </Modal>
...
export default Ecercisejumblevbprabodh
  ...
  backgroundImage: {
    ...
    resizeMode: 'cover',
      ...
      randomtext: {
        ...
        color: 'black',
        fontWeight: 'bold',
        marginTop: 10, // Adjust the top margin for space between texts
        lineHeight: 26, // Adjust the line height as needed
      },
    ...
    pressedWordsContainer: {
      backgroundColor: 'white',
      width: '90%',
      padding: 10,
      marginTop: 10,
      alignSelf: 'flex-start', // Align from the left
      marginLeft:'5%'
    },
    ...
    modalCloseButton: {
      ...
      paddingVertical: 10, // Add some vertical padding for better appearance
