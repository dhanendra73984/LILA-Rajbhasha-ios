...
const Narrativeprabodh = (props:any) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const hindiResponse = await axios.get(`https://lilaonmobile.rb-aai.in/LILAMobileData/VTT/${Package}${selectedLessonIndex}.vtt`);
        const englishResponse = await axios.get(`https://lilaonmobile.rb-aai.in/LILAMobileData/VTT/${Package}${Medium}${selectedLessonIndex}.vtt`);
        ...
        displaySubtitles(hindiSubtitleData);
  ...
  const parseSubtitleData = (subtitleText: string): SubtitleData[] => {
    ...
    return lines.reduce<SubtitleData[]>((acc, line, index, array) => {
      if (line.includes('-->')) {
        ...
        const duration = getSubtitleDuration(line);
        if (text && duration > 0) {
  ...
  const getSubtitleDuration = (timestampLine: string): number => {
    const [, startTime, endTime] = timestampLine.match(/(\d+:\d+:\d+\.\d+) --> (\d+:\d+:\d+\.\d+)/) || [];
 ...
 //model 
  ...
  return (
    ...
    <View>
        ...
        <VideoPlayer
          video={{ uri: `https://lilaonmobile.rb-aai.in/LILAMobileData/LessonsVideo/${Package.toLowerCase()}/${Package}${selectedLessonIndex}.mp4` }}
          videoWidth={1200}
          videoHeight={900}
          autoplay
        />
...
export default Narrativeprabodh;
