...
const Testproceedpraveen = (props: any) => {
  const { ApiResponse, Package, Medium, selectedLessonIndex, orignalessonindex,title, } = props.route.params;
            ...
            const filteredExercisesTables = testData
              .filter((item: { ExercisesTables: string | string[]; }) => 
                ...
                (
                  ...
                  item.ExercisesTables.includes("TestFill") || 
                  item.ExercisesTables.includes("TrueFalse")) &&
                  item.ExercisesTables !== `PraveentT${selectedLessonIndex}`
                )
              )
              .map((item: { ExercisesTables: string; }) => item.ExercisesTables.replace(new RegExp(`^PraveentT${selectedLessonIndex}`), ''));
  ...
  const handleProceedPress = () => {
          ...
          props.navigation.navigate('Teststartpraveen', { ApiResponse, Package, Medium, selectedLessonIndex, orignalessonindex,modifiedArray,originalExercisesTables });
...
export default Testproceedpraveen;
