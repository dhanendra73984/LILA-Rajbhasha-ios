...
const Testtrufalsestart = (props: any) => {
  const {
    ApiResponse,
    Package,
    Medium,
    selectedLessonIndex, orignalessonindex,
    title,
    modifiedArray,
  ...
  const modifiedArrayData = modifiedArray[2];
  const originalExercisesTablesData = originalExercisesTables[2];
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        requestData.tablename = originalExercisesTables[2];
...
// for fill  in the blanks 
  const handleNext = () => {
    ...
    if (currentIndex === testChoiceData.length - 1) {
      ...
      props.navigation.navigate('fillintheblanksecandtest', {
        ...
        testChoiceCounter2,
        trueFalseCounter2,
        trueFalseCounter3: trueFalseCounter,
        testChoiceCounter3:testChoiceCounter,
        fillInBlanksCounter3:fillInBlanksCounter,
  ...
  //header navigation
    ...
    const getInitialTimerValue = () => {
      if (modifiedArray.length > 0) {
        const firstModifiedArrayData = modifiedArray[2];
...
export default Testtrufalsestart;
